...
async def log_transfer(bot_client, log_channel, sent_message,
    ...
    if not log_channel or not sent_message or isinstance(sent_message, bool):
        return
    msg_id = sent_message if isinstance(sent_message, int) else sent_message.id
    try:
...
async def _ptb_send_media(
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            ...
            if is_photo:
                msg = await ptb_bot.send_photo(**common, photo=inp)

            elif is_video:
                msg = await ptb_bot.send_video(
                    **common,
                    video=inp,
                ...
                msg = await ptb_bot.send_document(**common, document=inp)
            ...
            try:
                ...
                if (is_photo or is_image):
                    try:
                        ...
                        if ptb_bot:
                            ptb_msg_id = await _ptb_send_media(
